...
def heritage(names: dict[str, str], isos: set[str]) -> list[str]:
    return list(heritage_values(names, isos))


def heritage_values(names: dict[str, str], isos: set[str]) -> dict[str, float]:
    ...
    for country, count in re.findall(
        ...
        if keep(iso, isos, found):
            found[iso] = float(count)  # type: ignore[index]
    return found


def olympics_values(isos: set[str]) -> dict[str, float]:
    text = wiki("All-time Olympic Games medal table", "wikitext")["wikitext"]["*"]
    found: dict[str, float] = {}
    for match in re.finditer(r"\{\{flag IOC\|([A-Z0-9]+)\}\}(.*?)(?=\n\|-|\n\|\})", text, re.S):
        code = match.group(1)
        ...
        if len(nums) < 5:
            continue
        found[iso] = float(nums[-4])  # type: ignore[index]
    return found


def gdp_values_from_visual(names: dict[str, str], isos: set[str]) -> dict[str, float]:
    text = (ROOT / "scripts/imf-weo-gdp-april-2026.txt").read_text()
    found: dict[str, float] = {}
    for match in re.finditer(
        ...
        iso = resolve(raw, names)
        if keep(iso, isos, found):
            found[iso] = float(match.group(2).replace(",", ""))  # type: ignore[index]
    return found


def population_values_from_passports(isos: set[str]) -> dict[str, float]:
    text = (ROOT / "src/data/passports.ts").read_text()
    found: dict[str, float] = {}
    for iso, pop in re.findall(r"^\s+([a-z]{2}): p\([^)]*?,\s*([\d_]+),", text, re.M):
        if iso in isos:
            found[iso] = float(int(pop.replace("_", "")))
    return found
    ...
    *,
    ...
    for row in rows:
        ...
        if not keep(iso, isos, found):
            continue
        value = None
        if col is not None and col < len(row) and "%" not in row[col]:
            value = parse_amount(row[col])
        else:
            for cell in row[1:]:
                if "%" in cell:
                ...
                if parsed is not None and lo <= parsed <= hi:
                    value = parsed
                    break
        if (value is None or not lo <= value <= hi) and inherit_last:
            value = last
        if value is None or not lo <= value <= hi:
            continue
        last = value
        found[iso] = value  # type: ignore[index]
    return found


def first_ranked_table_obj(tables: list[list[list[str]]], names: dict[str, str]) -> list[list[str]]:
    for table in tables:
        for row in table:
            if row and re.sub(r"[^\d]", "", row[0]) == "1" and row_iso(row, names):
                return table
...
def load_orders_ts() -> dict[str, list[str]]:
    text = (ROOT / "src/data/rankingOrders.ts").read_text()
    return {key: json.loads(raw) for key, raw in re.findall(r"  (\w+): (\[[^\]]+\])", text)}
...
def write_values_ts(values: dict[str, dict[str, float]]) -> None:
    lines = [
        "/** Generated by scripts/fetch-rankings.py — scores from the same sources as RANKING_ORDERS. */",
        "export const RANKING_VALUES = {",
    ]
    for key, by_iso in values.items():
        inner = ", ".join(f'"{iso}": {fmt_score(score)}' for iso, score in by_iso.items())
        lines.append(f"  {key}: {{{inner}}},")
    lines.append("} as { readonly [key: string]: { readonly [iso: string]: number } }")
    lines.append("")
    (ROOT / "src/data/rankingValues.ts").write_text("\n".join(lines))


def align_values(orders: dict[str, list[str]], raw: dict[str, dict[str, float]]) -> dict[str, dict[str, float]]:
    aligned: dict[str, dict[str, float]] = {}
    for key, isos in orders.items():
        scores = raw.get(key, {})
        aligned[key] = {iso: scores[iso] for iso in isos if iso in scores}
        missing = [iso for iso in isos if iso not in scores]
        print(f"{key}: {len(aligned[key])}/{len(isos)} values, missing {len(missing)}")
        if missing[:8]:
            print("  missing", missing[:8])
    return aligned


def collect_values(names: dict[str, str], isos: set[str]) -> dict[str, dict[str, float]]:
    values: dict[str, dict[str, float]] = {
        "rankGdp": gdp_values_from_visual(names, isos),
        "rankPopulation": population_values_from_passports(isos),
        "rankOlympics": olympics_values(isos),
        "rankHeritage": heritage_values(names, isos),
    }
    print("rankGdp", len(values["rankGdp"]))
    print("rankPopulation", len(values["rankPopulation"]))
    print("rankOlympics", len(values["rankOlympics"]))
    print("rankHeritage", len(values["rankHeritage"]))

    ranked = {
        "rankArea": ("List of countries and dependencies by area", 0.4, 20_000_000, False),
        "rankHappiness": ("World Happiness Report", 1.0, 8.5, False),
        "rankHdi": ("List of countries by Human Development Index", 0.3, 1.0, True),
        "rankCpi": ("Corruption Perceptions Index", 5, 100, False),
        "rankPassport": ("Henley Passport Index", 10, 200, True),
        "rankPeace": ("Global Peace Index", 1.0, 4.5, True),
        "rankBillionaires": ("List of countries by number of billionaires", 1, 2_000, True),
    }
    for key, (page, lo, hi, inherit) in ranked.items():
        tables = wiki_tables(page)
        table = first_ranked_table_obj(tables, names)
        values[key] = values_from_rows(table, names, isos, lo=lo, hi=hi, inherit_last=inherit)
        print(key, len(values[key]), page)

    ppp_tables = wiki_tables("List of countries by GDP (PPP)")
    ppp_table = ppp_tables[0] if ppp_tables else []
    ppp_millions = values_from_rows(ppp_table, names, isos, col=1, lo=10, hi=1e8)
    for col in (2, 3):
        extra = values_from_rows(ppp_table, names, isos, col=col, lo=10, hi=1e8)
        for iso, millions in extra.items():
            ppp_millions.setdefault(iso, millions)
    values["rankGdpPpp"] = {iso: millions / 1000 for iso, millions in ppp_millions.items()}
    print("rankGdpPpp", len(values["rankGdpPpp"]))

    pc_tables = wiki_tables("List of countries by GDP (nominal) per capita")
    values["rankGdpPc"] = values_from_rows(pc_tables[0] if pc_tables else [], names, isos, col=1, lo=100, hi=300_000)
    print("rankGdpPc", len(values["rankGdpPc"]))

    life_tables = wiki_tables("List of countries by life expectancy")
    life_table = next(
        (t for t in life_tables if len(t) > 100 and row_iso(t[2] if len(t) > 2 else t[0], names) == "jp"),
        None,
    )
    if life_table is None:
        life_table = max(life_tables, key=len, default=[])
    values["rankLife"] = values_from_rows(life_table, names, isos, lo=40, hi=90)
    print("rankLife", len(values["rankLife"]))

    press_tables = wiki_tables("World Press Freedom Index")
    values["rankPress"] = values_from_rows(press_tables[0] if press_tables else [], names, isos, lo=5, hi=100)
    print("rankPress", len(values["rankPress"]))

    co2_tables = wiki_tables("List of countries by carbon dioxide emissions")
    values["rankCo2"] = values_from_rows(co2_tables[0] if co2_tables else [], names, isos, lo=0.01, hi=20_000)
    print("rankCo2", len(values["rankCo2"]))

    gini_tables = wiki_tables("List of countries by income inequality")
    gini: dict[str, float] = {}
    for row in gini_tables[0][2:] if gini_tables else []:
        iso = row_iso(row, names)
        if not keep(iso, isos, gini) or len(row) < 6:
            continue
        value = parse_amount(row[3]) or parse_amount(row[5])
        if value is None or value < 15 or value > 80:
            continue
        gini[iso] = value  # type: ignore[index]
    values["rankGini"] = gini
    print("rankGini", len(values["rankGini"]))

    mil_tables = wiki_tables("List of countries by number of millionaires")
    mil: dict[str, float] = {}
    seen: set[str] = set()
    best_table: list[list[str]] = []
    for table in mil_tables:
        countries = sum(1 for row in table[1:] if any(resolve(cell, names) for cell in row))
        if countries > len(best_table):
            best_table = table
    for row in best_table[1:]:
        iso = row_iso(row, names)
        ...
        for cell in row:
            parsed = parse_amount(cell)
            if parsed is not None and 1_000 <= parsed <= 1e9:
                value = parsed
                break
        if value is None:
            continue
        seen.add(iso)  # type: ignore[arg-type]
        mil[iso] = value  # type: ignore[index]
    values["rankMillionaires"] = mil
    print("rankMillionaires", len(values["rankMillionaires"]))
    return values


def write_values_only() -> None:
    names, isos = load_countries()
    orders = load_orders_ts()
    aligned = align_values(orders, collect_values(names, isos))
    write_values_ts(aligned)
    (ROOT / "scripts" / "ranking-values.json").write_text(json.dumps(aligned, ensure_ascii=False, indent=2))
    print("wrote src/data/rankingValues.ts")
